...
- **Parameters**: `prompt` (required), `options` (required — string array or null for free-text only), `showAdditionalFreeInputButton` (required boolean), `title` (required — string or null), `preExpandTextInputBox` (required boolean)
- **Timeout**: No default timeout (configurable via `USER_INPUT_TIMEOUT_MINUTES` env var)
...
- **`preExpandTextInputBox`** (required): If true, pre-selects the "Other..." free-text option so the UI loads with the input box open. Use when options are useful but secondary to free text input. Set false otherwise.
...
- **Two-tool pattern**: After calling this tool, you MUST call `user_input_inline__await_presented_ui` with the returned `requestId` to get the user's response.

### `user_input_inline__await_presented_ui`
Waits for and retrieves the user's response from a `user_input_inline__present_ui` call.
- **Parameters**: `requestId` (required) - the requestId returned by `user_input_inline__present_ui`
